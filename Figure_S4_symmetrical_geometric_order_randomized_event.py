...
for r in range(grid_rows):
    for c in range(grid_cols):
        # introduce jitter to avoid perfect grid alignment
...
for step in range(T_steps + 1):
    ...
    for src in sources:
        ...
        for side, d in dists.items():
            ...
            if d > 0 and prop_radius >= d and not src["triggered"][side]:
                # Block self-triggering to prevent infinite loops at boundary
                if src["parent_side"] == side: continue 
                
                src["triggered"][side] = True
                sx, sy = src["pos"]
                
                # Regenerate at the point of contact 
    ...
    if step in capture_steps:
        # Sum surfaces from all currently active sources
